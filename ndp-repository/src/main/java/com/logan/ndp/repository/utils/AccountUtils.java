package com.logan.ndp.repository.utils;
...
//    }
    ...
    public <T> T getAccountById(Integer sendAccountId, Class<T> clazz) {
        try {
            Optional<ChannelAccountDo> optionalChannelAccount = channelAccountDao.findById(Long.valueOf(sendAccountId));
            if (optionalChannelAccount.isPresent()) {
                ChannelAccountDo channelAccount = optionalChannelAccount.get();
